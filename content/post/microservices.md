...
Microservices as an architecture style of building systems has gained popularity in recent years because of its benefits [^3][^1]. The proliferation of case studies and the development of robust supporting infrastructure contribute to making microservices a reliable choice. Notable examples include the works cited in references [^8], [^7], and [^9].
...
* **Isolation of Source Code**: Isolating source code can be beneficial for two reasons: It isolates experiments with new approaches or libraries, and it isolates bad code designs [^6][^4]. The isolation helps to safely replace the service with a better version due to all dependencies on that service being on its REST API and not on the code directly.
* **Reducing impact of Faults**: The failure of a single microservice, whether due to a memory leak or excessive CPU utilization, would not compromise the availability of other features within the system that lack a significant dependency on that microservice. This characteristic enhances the overall resilience of the architecture.

# Challenges Ahead

Due to the extensive availability of case studies and the maturation of the microservices approach, a number of well-documented challenges have emerged in its adoption. This section will explore several of these challenges in detail.
...
Achieving this outcome necessitates substantial refactoring efforts and a collaborative commitment across teams to this approach, which may require a considerable amount of time to fully implement.
